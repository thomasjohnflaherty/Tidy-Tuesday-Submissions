...
```{r}
path <- "C:/Users/tflaherty/Downloads/EDULIT_DS_10052019145625752.csv"
...
df <- read_csv(path) %>% 
  ...
  group_by(country) %>% 
  filter(sum(is.na(student_ratio)) < 20) %>% 
...
df %>%
  ...
  geom_tile(aes(y = student_ratio / 2, height = student_ratio), width = 0.9, show.legend = F) +
